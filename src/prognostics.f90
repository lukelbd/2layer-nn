!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Solve prognostic vorticity equation in spectral coordinates
! Seems to apply forcing terms to both *vort* and *qymean* -- the anomaly
! and mean components.
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
module prognostics
contains
subroutine prog
  use global_variables
  use mkl_dfti
  use mkl_trig_transforms

  implicit none
  integer :: i,j,i1,j1,n1,m1,ii,jj
  real :: ell,rkk,kkk,filter

  ! ------ Compute energy -------
  energy2 = 0.5*sum(vxy1*vxy1 + vxy2*vxy2 +  &
    uxy1*uxy1 + uxy2*uxy2)/float(imax*(jmax+1))

  do j = 2,jtrunc
    !    ---- Solve prognostic vorticity in spectral coordiantes ----
    ell = el*float(j-1)
    !    if(t.eq.tstart) then
    !      vqm_1(j,2) = vqm_1(j,3)
    !      vqm_1(j,1) = vqm_1(j,2)
    !      vqm_2(j,2) = vqm_2(j,3)
    !      vqm_2(j,1) = vqm_2(j,2)
    !    endif
    do i = 2,itrunc
      rkk = rk*float(i-1)
      ! Get *hyperdiffusion*, *radiation*, and *friction* terms
      visc_1(i,j) = -damp*((rkk**2+ell**2)**3)*vort_1(i,j,3)
      visc_2(i,j) = -damp*((rkk**2+ell**2)**3)*vort_2(i,j,3)
      rad_1(i,j)  = -(psi_2(i,j)-psi_1(i,j))/((rd*rd)*tau_r)
      rad_2(i,j)  = (psi_2(i,j)-psi_1(i,j))/((rd*rd)*tau_r)
      fric_2(i,j) = (rkk**2+ell**2)*psi_2(i,j)/tau_f
      !    fric_2(i,j) = fric_2(i,j)-vort_2(i,j,3)/tau_2
      if(t.eq.tstart) then
        adv_1(i,j,2) = adv_1(i,j,3)
        adv_1(i,j,1) = adv_1(i,j,2)
        adv_2(i,j,2) = adv_2(i,j,3)
        adv_2(i,j,1) = adv_2(i,j,2)
      endif
      ! Apply *hyperdiffusion* and *forcing* in upper layer
      vort_1(i,j,4) = vort_1(i,j,3) &
        + dt*(23.*adv_1(i,j,3)-16.*adv_1(i,j,2)+5.*adv_1(i,j,1))/12. &
        + dt*visc_1(i,j) &
        + dt*force_1(i,j) &
        + dt*rad_1(i,j)
      ! Apply *hyperdiffusion*, *forcing*, and *friction* in lower layer
      vort_2(i,j,4) = vort_2(i,j,3) +     &
        dt*(23.*adv_2(i,j,3)-16.*adv_2(i,j,2)+5.*adv_2(i,j,1))/12.
      vort_2(i,j,4) = vort_2(i,j,4) + dt*visc_2(i,j)
      vort_2(i,j,4) = vort_2(i,j,4) + dt*rad_2(i,j)
      vort_2(i,j,4) = vort_2(i,j,4) + dt*fric_2(i,j)
    enddo
    !    ---- Apply truncation ----
    do i = itrunc+1,idft
      vort_1(i,j,4) = zero
      vort_2(i,j,4) = zero
    enddo
    do i = 1,1
      vort_1(i,j,4) = zero
      vort_2(i,j,4) = zero
    enddo

    !    ---- Prognostic meridional derivative of zonal mean vorticity ----
    ! Iterate, apply *hyperdiffusion* and *forcing* in upper layer
    qymean1(j,4) = qymean1(j,3) -  &
      dt*(23.*vqm_1(j,3)-16.*vqm_1(j,2)+5.*vqm_1(j,1))/12.
    qymean1(j,4) = qymean1(j,4) - dt*damp*(ell**6)*qymean1(j,3)
    qymean1(j,4) = qymean1(j,4) - dt*(umean1(j)-umean2(j))/(rd*rd*tau_r)
    ! Iterate, apply *hyperdiffusion*, *forcing*, and *friction* in lower layer
    qymean2(j,4) = qymean2(j,3) -  &
      dt*(23.*vqm_2(j,3)-16.*vqm_2(j,2)+5.*vqm_2(j,1))/12.
    qymean2(j,4) = qymean2(j,4) - dt*damp*(ell**6)*qymean2(j,3)
    qymean2(j,4) = qymean2(j,4) + dt*(umean1(j)-umean2(j))/(rd*rd*tau_r)
    qymean2(j,4) = qymean2(j,4) - dt*(ell*ell*umean2(j))/tau_f
  enddo

  !    ---- Apply truncation ----
  do j = jtrunc+1,jmax+1
    vort_1(:,j,4) = zero
    vort_2(:,j,4) = zero
    qymean1(j,4) = 0.
    qymean2(j,4) = 0.
  enddo

  return
end subroutine
end module
